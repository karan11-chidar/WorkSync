...
export default function LeaveHeader() {
  const { filterStatus, setFilterStatus, counts, refreshLeaves } =
  ...
  return (
    <div className="w-full p-1">
      <div
        className="bg-white p-6 rounded-2xl border border-slate-100 shadow-2xs flex flex-col xl:flex-row xl:items-center justify-between gap-5"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Right Side: Quick Filter Selectors */}
          <div className="grid grid-cols-2 lg:grid-cols-4 items-center gap-2.5 bg-slate-50 px-2 py-2 rounded-xl border border-slate-100 self-start xl:self-auto w-full xl:w-auto">
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center justify-between gap-2 px-3 py-1.5 text-xs rounded-lg font-semibold transition-all duration-200 cursor-pointer active:scale-95 ${
                    isActive
                      ? option.activeColor
                      : "bg-transparent text-slate-600 hover:bg-white hover:text-slate-900 hover:shadow-2xs"
                  }`}
